...
export const generateArticle = async (req, res) => {
    try {
        ...
        // if user does not have premium plan and also used 10 free credits then
    ...
    if (!prompt || !length) {
      return res.status(400).json({ success: false, message: "Missing prompt or length" });
        ...
        // storing the result in content
        const content=response.choices[0].message.content;
        
        // updating database
        await sql `INSERT INTO creations (user_id,prompt,content,type) VALUES (${userId}, ${prompt},${content},'article')`;
        
        // if plan is not premium then we have increase free usage by 1
        if(plan!=='premium'){
        ...
        // sending response to frontend having content
...
export const removeImageBackground=async (req,res)=>{
   try{
    ...
    const {userId}=req.auth();
    const image=req.file;//to store image in the req.file, we use multer
    const plan=req.plan;
...
export const resumeReview=async(req,res)=>{
    try{
        ...
        // now we have to parse the resume to extract its text by using pdf-parse package
