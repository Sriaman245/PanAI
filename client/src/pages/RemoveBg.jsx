...
const RemoveBg = () => {
  ...
  const onSubmitHandler=async(e)=>{
    ...
    try{
      ...
      // in this we have to upload image
      const formData=new FormData()
      formData.append('image',input)

      const {data}=await axios.post('/api/ai/remove-image-bg',formData,{headers:{Authorization:`Bearer ${ await getToken()}`}})
  ...
  return (
     <div className='h-full overflow-y-scroll p-6 flex items-start flex-wrap gap-4 text-slate-700'>
      {/* left col */}
      <form onSubmit={onSubmitHandler} className='w-full max-w-lg p-4 bg-white rounded-lg border-gray-200'>
        ...
        {/* setInput(e.target.files[0])} - the first file selected by the user */}
